...
| REST/WS API server   | ✅     | apiserver v1: REST + SSE + **WebSocket event bridge** (send/sync/subscribe, bearer auth, /metrics gauge) + sealed storage + `POST /v1/pair` (mock companion; live WhatsApp still pending) |
