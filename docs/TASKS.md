...
| T-103 | Registration & QR/pairing-code flow (companion platform) | IN_PROGRESS | mock companion flow DONE: CLI (`batur pair --mock --data DIR`, resumed by `serve --mock`) and `POST /v1/pair` (attaches the session). Live WhatsApp companion registration still needs a real device. |
...
| T-204 | Postgres/SQLite storage adapters (multi-node) | IN_PROGRESS | ADR-0007; sibling `storage-adapters/` module scaffolded (postgres + sqlite stubs return ErrNotImplemented until a driver is chosen; CI lane builds + tests it). Real schema + driver selection still pending. |
